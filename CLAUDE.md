...
- **CI/CD mis en place** : dépôt Git dédié, `lel-site-ci4/` comme racine (l'ancien site procédural du dossier parent n'est pas versionné, dépôt GitHub public). Un push sur `main` déclenche `.github/workflows/deploy.yml` : `composer install --no-dev` sur le runner puis déploiement automatique par SFTP vers `/sites/enfants-lumieres/lel-site-ci4/` sur Infomaniak (remplace le dépôt FTP manuel qui reste documenté en fallback dans le `README.md`). Les migrations de base de données restent manuelles, volontairement non automatisées dans le pipeline.
