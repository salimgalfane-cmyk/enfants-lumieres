...
- **Anti-spam sur le formulaire de contact** : honeypot natif CodeIgniter (champ invisible auto-injecté par le filtre `honeypot`, scopé à la seule route `contact` dans `Config/Filters.php`, rejette les bots qui le remplissent) et limitation de débit via le Throttler natif (`service('throttler')`, 3 messages / 10 min par IP, clé hachée en MD5 pour éviter une exception sur les adresses IPv6 qui contiennent des `:`, caractère réservé du cache fichier). Nouvelle table `ips_bloquees` (IP en clair, comme `messages_contact.ip_origine` déjà existant) : toute IP bloquée est rejetée dès `Contact::store()`, avant validation. Admin : sur la liste des messages, boutons « Bloquer l'IP » (bloque et supprime le message en un clic) et « Supprimer » (suppression simple) ; page dédiée `/admin/ips-bloquees` pour voir et débloquer. Migration `CreateIpsBloqueesMigration`.
